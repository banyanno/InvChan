Public Class Search_Issue
    ...
    Dim PIssue As UC_RptIssue

    Dim drag As Boolean
    Dim mousex As Integer
    Dim mousey As Integer
    Sub New(ByVal MyPanel As UC_RptIssue)
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        If Me.RadSum.Checked = True Then
            Dim DAIssue As New DSReportTableAdapters.ISSUE_SUMTableAdapter
            Dim ResultTable As New DataTable
            Dim MyReport As New RptIssue_Sum
            Dim ReportViewer As New Report_Viewer
            If Me.CboLocation.Text = "All Locations" Then
                If RadByAll.Checked = True Then
                    ResultTable = DAIssue.SelectAllLocationByDate(FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                ElseIf RadByItem.Checked = True Then
                    ResultTable = DAIssue.SelectAllocationByItemIDAndDate(CInt(Me.CboItem.SelectedValue), FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                Else
                    ResultTable = DAIssue.SelectAllLocationByCategoryAndDate(Me.cboCategory.Text, FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                End If
            Else
                If RadByAll.Checked = True Then
                    ResultTable = DAIssue.SelectByDate(Me.CboLocation.Text, FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                ElseIf RadByItem.Checked = True Then
                    ResultTable = DAIssue.SelectByItemIDAndDate(Me.CboLocation.Text, CInt(Me.CboItem.SelectedValue), FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                Else
                    ResultTable = DAIssue.SelectByCategoryAndDate(Me.CboLocation.Text, Me.cboCategory.Text, FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                End If
            End If
            If ResultTable.Rows.Count <= 0 Then
                MsgBox("No record was found", MsgBoxStyle.Information)
            Else
                PIssue.IssueList.DataSource = ResultTable
        ...
        Else
            Dim ResultTable As New DataTable
            Dim MyReport As New RptIssue
            Dim ReportViewer As New Report_Viewer
            If Me.CboLocation.Text = "All Locations" Then
                If RadByAll.Checked = True Then
                    ResultTable = DAIssue.SelectAllLocationByDate(FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                ElseIf RadByItem.Checked = True Then
                    ResultTable = DAIssue.SelectAllLocationsByItemIDAndDate(CInt(Me.CboItem.SelectedValue), FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                Else
                    ResultTable = DAIssue.SelectAllLocationByCategoryAndDate(Me.cboCategory.Text, FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
                End If
            Else
