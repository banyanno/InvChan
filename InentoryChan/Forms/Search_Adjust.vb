Public Class Search_Adjust
    ...
    Dim PAdjust As UC_RptAdjust

    Dim drag As Boolean
    Dim mousex As Integer
    Dim mousey As Integer
    Sub New(ByVal MyPanel As UC_RptAdjust)
    ...
    Private Sub btnSearch_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSearch.Click
        ...
        Dim MyReport As New RptAdjust
        Dim ReportViewer As New Report_Viewer
        If RadByAll.Checked = True Then
            ResultTable = DAAdjust.SelectByDate(FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
        ElseIf RadByItem.Checked = True Then
            ResultTable = DAAdjust.SelectByItemID(CInt(Me.CboItem.SelectedValue), FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
        Else
            ResultTable = DAAdjust.SelectByCategoryName(Me.cboCategory.Text, FormatDateTime(Me.DtFrom.Value, DateFormat.ShortDate), FormatDateTime(Me.DtTo.Value, DateFormat.ShortDate))
        End If
        If ResultTable.Rows.Count <= 0 Then
            MsgBox("No record was found", MsgBoxStyle.Information)
        Else
            PAdjust.AdjustList.DataSource = ResultTable
