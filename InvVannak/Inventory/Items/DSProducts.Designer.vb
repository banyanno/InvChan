    ...
    Partial Public Class ITEMSTableAdapter
        ...
        Private Sub InitAdapter()
            ...
            Me._adapter.InsertCommand.CommandText = "INSERT INTO [dbo].[ITEMS] ([ITEM_BARCODE], [ITEM_NAME], [CATE_ID], [ITEM_IMAGE], "& _ 
                ...
                "(SELECT CATE_NAME FROM CATEGORIES AS C WHERE (CATE_ID = I.CATE_ID)) AS CATE_NAME"& _ 
                ", (SELECT CONVERT (varchar(25), BALANCE) AS Expr1 FROM ITEM_BALANCE AS B WHERE ("& _ 
                "ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Main Unit')) + ' ' + (SELECT UNIT_NAME FR"& _ 
                "OM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Main Unit')) "& _ 
                "AS MAIN_BALANCE, (SELECT UNIT_NAME FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITE"& _ 
                "M_ID) AND (UNIT_TYPE = 'Main Unit')) AS MAIN_UNIT, (SELECT BALANCE FROM ITEM_BAL"& _ 
                "ANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Main Unit')) AS MAIN_QTY"& _ 
                ", (SELECT BALANCE FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_T"& _ 
                "YPE = 'Sub Unit')) AS SUB_QTY, (SELECT RATIO_QTY FROM ITEM_BALANCE AS B WHERE (I"& _ 
                "TEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Main Unit')) AS MAIN_RATIO, (SELECT RATIO_"& _ 
                "QTY FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Sub Uni"& _ 
                "t')) AS SUB_RATIO, (SELECT CONVERT (varchar(25), BALANCE) AS Expr1 FROM ITEM_BAL"& _ 
                "ANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Sub Unit')) + ' ' + (SEL"& _ 
                "ECT UNIT_NAME FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE "& _ 
                "= 'Sub Unit')) AS SUB_BALANCE, (SELECT UNIT_NAME FROM ITEM_BALANCE AS B WHERE (I"& _ 
                "TEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Sub Unit')) AS SUB_UNIT, (SELECT USD_COST "& _ 
                "FROM ITEM_COSTS AS C WHERE (ITEM_ID = I.ITEM_ID) AND (IS_LAST = 1)) AS USD_COST,"& _ 
                " (SELECT KHR_COST FROM ITEM_COSTS AS C WHERE (ITEM_ID = I.ITEM_ID) AND (IS_LAST "& _ 
                "= 1)) AS KHR_COST, (SELECT THB_COST FROM ITEM_COSTS AS C WHERE (ITEM_ID = I.ITEM"& _ 
                "_ID) AND (IS_LAST = 1)) AS THB_COST, (SELECT USD_PRICE FROM ITEM_PRICES AS P WHE"& _ 
                "RE (ITEM_ID = I.ITEM_ID) AND (IS_LAST = 1)) AS USD_PRICE, (SELECT KHR_PRICE FROM"& _ 
                " ITEM_PRICES AS P WHERE (ITEM_ID = I.ITEM_ID) AND (IS_LAST = 1)) AS KHR_PRICE, ("& _ 
                "SELECT THB_PRICE FROM ITEM_PRICES AS P WHERE (ITEM_ID = I.ITEM_ID) AND (IS_LAST "& _ 
                "= 1)) AS THB_PRICE, (SELECT RETAIL_PRICE FROM ITEM_PRICES AS P WHERE (ITEM_ID = "& _ 
                "I.ITEM_ID) AND (IS_LAST = 1)) AS RETAIL_PRICE, ITEM_IMAGE, USER_ID, DATE_CREATE "& _ 
                "FROM ITEMS AS I WHERE (ITEM_ID = SCOPE_IDENTITY()) ORDER BY ITEM_NAME"
            ...
            Me._adapter.UpdateCommand.CommandText = "UPDATE [dbo].[ITEMS] SET [ITEM_BARCODE] = @ITEM_BARCODE, [ITEM_NAME] = @ITEM_NAME"& _ 
                ...
                "DE, ITEM_NAME, CATE_ID, (SELECT CATE_NAME FROM CATEGORIES AS C WHERE (CATE_ID = "& _ 
                "I.CATE_ID)) AS CATE_NAME, (SELECT CONVERT (varchar(25), BALANCE) AS Expr1 FROM I"& _ 
                "TEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Main Unit')) + ' "& _ 
                "' + (SELECT UNIT_NAME FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UN"& _ 
                "IT_TYPE = 'Main Unit')) AS MAIN_BALANCE, (SELECT UNIT_NAME FROM ITEM_BALANCE AS "& _ 
                "B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Main Unit')) AS MAIN_UNIT, (SELE"& _ 
                "CT BALANCE FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = '"& _ 
                "Main Unit')) AS MAIN_QTY, (SELECT BALANCE FROM ITEM_BALANCE AS B WHERE (ITEM_ID "& _ 
                "= I.ITEM_ID) AND (UNIT_TYPE = 'Sub Unit')) AS SUB_QTY, (SELECT RATIO_QTY FROM IT"& _ 
                "EM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Main Unit')) AS MA"& _ 
                "IN_RATIO, (SELECT RATIO_QTY FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) A"& _ 
                "ND (UNIT_TYPE = 'Sub Unit')) AS SUB_RATIO, (SELECT CONVERT (varchar(25), BALANCE"& _ 
                ") AS Expr1 FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = '"& _ 
                "Sub Unit')) + ' ' + (SELECT UNIT_NAME FROM ITEM_BALANCE AS B WHERE (ITEM_ID = I."& _ 
                "ITEM_ID) AND (UNIT_TYPE = 'Sub Unit')) AS SUB_BALANCE, (SELECT UNIT_NAME FROM IT"& _ 
                "EM_BALANCE AS B WHERE (ITEM_ID = I.ITEM_ID) AND (UNIT_TYPE = 'Sub Unit')) AS SUB"& _ 
                "_UNIT, (SELECT USD_COST FROM ITEM_COSTS AS C WHERE (ITEM_ID = I.ITEM_ID) AND (IS"& _ 
                "_LAST = 1)) AS USD_COST, (SELECT KHR_COST FROM ITEM_COSTS AS C WHERE (ITEM_ID = "& _ 
                "I.ITEM_ID) AND (IS_LAST = 1)) AS KHR_COST, (SELECT THB_COST FROM ITEM_COSTS AS C"& _ 
                " WHERE (ITEM_ID = I.ITEM_ID) AND (IS_LAST = 1)) AS THB_COST, (SELECT USD_PRICE F"& _ 
                "ROM ITEM_PRICES AS P WHERE (ITEM_ID = I.ITEM_ID) AND (IS_LAST = 1)) AS USD_PRICE"& _ 
                ", (SELECT KHR_PRICE FROM ITEM_PRICES AS P WHERE (ITEM_ID = I.ITEM_ID) AND (IS_LA"& _ 
                "ST = 1)) AS KHR_PRICE, (SELECT THB_PRICE FROM ITEM_PRICES AS P WHERE (ITEM_ID = "& _ 
                "I.ITEM_ID) AND (IS_LAST = 1)) AS THB_PRICE, (SELECT RETAIL_PRICE FROM ITEM_PRICE"& _ 
                "S AS P WHERE (ITEM_ID = I.ITEM_ID) AND (IS_LAST = 1)) AS RETAIL_PRICE, ITEM_IMAG"& _ 
                "E, USER_ID, DATE_CREATE FROM ITEMS AS I WHERE (ITEM_ID = @ITEM_ID) ORDER BY ITEM"& _ 
                "_NAME"
        ...
        Private Sub InitCommandCollection()
            ...
            Me._commandCollection(0).CommandText = "SELECT I.ITEM_ID, I.ITEM_BARCODE, I.ITEM_NAME, I.CATE_ID,(SELECT CATE_NAME FROM C"& _ 
